...
from utils import load_state_dict_from_url #.utils
...
class MobileNetV2_1(nn.Module):
    def __init__(self,
        ...
        features = [ConvBNReLU(3, input_channel, stride=1)] #3 #stride=2
...
class MobileNetV2(nn.Module):
    def __init__(self,
                 ...
                 inverted_residual_setting3=None,
        ...
        if inverted_residual_setting3 is None:
            inverted_residual_setting3 = [
                # t, c, n, s
                [6, 80, 3, 2],
                [6, 160, 1, 1],
            ]
        ...
        input_channel = _make_divisible(input_channel * width_mult, round_nearest)
        self.last_channel = _make_divisible(last_channel * max(1.0, width_mult), round_nearest)
        features1 = [ConvBNReLU(3, input_channel, stride=1)] #3 #stride=2
        # building inverted residual blocks
        for t, c, n, s in inverted_residual_setting1:
            ...
            for i in range(n):
                ...
                input_channel = output_channel
        # building last several layers
        #features1.append(ConvBNReLU(input_channel, self.last_channel, kernel_size=1))
        ...
        for t, c, n, s in inverted_residual_setting2:
            ...
            for i in range(n):
                ...
                input_channel = output_channel
        # building last several layers
        #features2.append(ConvBNReLU(input_channel, self.last_channel, kernel_size=1))
        ...
        for t, c, n, s in inverted_residual_setting3:
            ...
            for i in range(n):
                ...
                input_channel = output_channel
        # building last several layers
        features3.append(ConvBNReLU(input_channel, self.last_channel, kernel_size=1))
    ...
    def _forward_impl(self, x):
        ...
        # Cannot use "squeeze" as batch-size can be 1 => must use reshape with x.shape[0]
        x = nn.functional.adaptive_avg_pool2d(x3, 1).reshape(x.shape[0], -1)
        x = self.classifier(x)
        return x, x1, x2, x3
...
def mobilenet_v21(pretrained=False, progress=True, **kwargs):
    """
    Constructs a MobileNetV2 architecture from
    `"MobileNetV2: Inverted Residuals and Linear Bottlenecks" <https://arxiv.org/abs/1801.04381>`_.
