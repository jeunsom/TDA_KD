...
def create_cnn_model(name, dataset="cifar100", use_cuda=False, num_cls=100):
	"""
	...
	if is_resnet(name): # or is_wide_resnet(name)
		...
		if dataset == 'cifar100' or dataset == 'cifar10':
			resnet_model = resnet_dict.get(resnet_size)(num_classes=num_classes)
		else:
		...
		wrn_size = name[3:]
		if dataset == 'cifar100' or dataset == 'cifar10':
			wrn_model = wrn_dict.get(wrn_size)(num_classes=num_classes)
		else:
		...
		if dataset == 'cifar100' or dataset == 'cifar10':
			mobile_model = mobilenet_book.get(mobilenet_size)(num_classes=num_classes)
		else:
